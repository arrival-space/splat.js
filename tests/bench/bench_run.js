...
const TAG = `${SET}_${ITERS}` + (Q.has('classic') ? '_classic' : '')
  ...
  + (Q.get('gspread') ? `_gs${Q.get('gspread')}` : '') + (Q.get('sgagg') ? '_sg' : '') + (Q.get('tilegrad') === '0' ? '_ntg' : '') + (Q.get('frommodel') ? '_fm' : '') + (Q.get('aspect') ? '_asp' : '') + (Q.get('asplr') ? `_al${Q.get('asplr')}` : '') + (Q.get('sfmaspect') ? '_sa' : '')
